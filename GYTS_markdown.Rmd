...
state_usage_total <- read_csv('./SQL_imports/GYTS4_state_usage_total.csv')
```
...
state_ur <- read.csv('./SQL_imports/state_ur.csv')
```
```{r echo=TRUE}
...
state_ecig <- read.csv('./SQL_imports/state_ecig.csv')
```
```{r}
...
```{r echo=FALSE}
knitr::include_graphics('./Map Output/Current Tobacco users (Pretty Breaks).png')
knitr::include_graphics('./Map Output/Current Tobacco users(Jenks).png')
```
That's all, folks.